...
Metro是一个受到牛腩小镇服务器启发的地铁交通系统插件。该插件允许管理员创建和管理自动化的地铁线路网络，并为玩家提供便捷、信息化的乘车体验。
...
* **线路管理**:
    * 创建、删除、重命名和管理地铁线路。
    * 每条线路拥有唯一的内部ID和玩家可见的显示名称，ID用于系统内部标识，显示名称可随时修改。
    * 每条线路包含一个按顺序排列的停靠区ID (`stop_id`)列表，定义了列车的行驶路径。
    * **线路颜色**: 每条线路可配置独特的颜色代码，用于在UI和计分板中直观显示线路标识。
    * **终点方向**: 支持为每条线路配置终点方向描述（如"星海总站方向"），增强导航信息。
* **停靠区管理**:
    * 创建、删除、重命名和管理地铁停靠区 (`stop`)。一个停靠区代表一个逻辑上的站台区域。
    * 每个停靠区拥有唯一的内部ID和玩家可见的显示名称。显示名称可随时修改而不影响系统功能。
    * 管理员可以通过为不同的停靠区设置相似的显示名称（例如，"主城广场 - 东行区"，"主城广场 - 西行区"）来表示它们同属于一个逻辑站点。
    ...
    * **自定义Title**: 每个停靠区可以设置独特的Title显示内容，覆盖全局配置，为不同站点提供个性化的信息展示。
...
* **自定义信息显示**:
    ...
    * **停靠区独立配置:** 每个停靠区可以设置独立的Title/Subtitle/ActionBar显示内容，覆盖全局配置，实现站点个性化。
...
* **线路管理:**
    * `/m line create <line_id> <"显示名称">`: 创建新线路，ID为系统内部唯一标识，显示名称为玩家可见的名称。
    * `/m line delete <line_id>`: 删除指定ID的线路。
    * `/m line list`: 列出所有已创建的线路。
    * `/m line setcolor <line_id> <颜色>`: 设置线路的显示颜色，使用Minecraft颜色代码（如&9表示蓝色）。
    * `/m line setterminus <line_id> <"终点方向名称">`: 设置线路的终点方向描述文本。
    * `/m line rename <line_id> <"新名称">`: 修改线路的显示名称，ID保持不变，不影响线路关联。
    * `/m line addstop <line_id> <stop_id> [顺序索引]`: 将停靠区加入线路，可指定插入位置。
...
* **系统:**
    ...
    * `/m testendpoint`: 测试终点站提示效果。
    * `/m teststopinfo <line_id> [stop_id]`: 测试线路或特定停靠区的信息显示。
...
* **`lines.yml`**:
    * 存储所有线路定义。
    * 每条线路包含其ID、显示名称、颜色代码、终点方向和按顺序排列的 `stop_id` 列表。
    * 示例:
        ```yaml
        line_1_eastbound:
          display_name: "1号线 (东行)"
...
* **线路ID**: 建议使用有意义的标识如 `line_1_eastbound`（1号线东行）, `line_circle`（环线）等。
* **停靠区ID**: 建议使用地点相关的标识如 `central_east`（中央站东行）, `market_north`（市场站北行）等。
* **显示名称**: 可以使用更友好的名称如 "1号线 - 东城方向", "中央商务区站 - 北行站台" 等。
